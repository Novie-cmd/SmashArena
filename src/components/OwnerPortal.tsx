  ...
  return (
    <div className="space-y-6" id="owner-portal-layout">
      ...
      <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-xl flex flex-col md:flex-row items-center justify-between gap-6" id="owner-barcode-section">
        ...
        <div className="border border-slate-800 bg-white p-4 rounded-xl flex flex-col items-center justify-center text-slate-950 shadow-2xl relative shrink-0">
          <div className="border-4 border-slate-105 p-1.5 bg-white rounded-lg">
            {typeof window !== 'undefined' ? (
              <img 
                ...
                className="w-28 h-28 cursor-pointer hover:opacity-90 transition-opacity"
                referrerPolicy="no-referrer"
                onClick={() => {
                  const url = `${window.location.origin}${window.location.pathname}?portal=customer`;
                  window.open(url, '_blank');
                }}
                title="Sangat scannable oleh HP Anda! Klik untuk buka tab baru."
              />
            ) : (
          ...
          <span className="text-[8px] font-semibold text-slate-600 mt-0.5 text-center">
            PINDAI UNTUK SEWA SEKARANG
          </span>
        </div>
      </div>
